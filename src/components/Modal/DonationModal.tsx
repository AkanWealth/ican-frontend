...
import React, { useState } from "react";
import { X } from "lucide-react";
// Import FlutterModal but comment out for now
// import FlutterModal from "@/components/genui/FlutterModal";
import donationService from "@/services/donationService";
import { useToast } from "@/hooks/use-toast";
...
const DonationModal: React.FC<DonationModalProps> = ({
  ...
  */
  ...
  // Temporary function to show payment unavailable message
  const handleTemporaryPayment = () => {
    toast({
      title: "Payment Not Available",
      description: "Online payment is not available at the moment. Please check back later.",
      variant: "destructive",
      duration: 5000,
    });
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-[720px] relative">
        ...
        <div className="pt-4 space-y-4">
          {/* Commented out FlutterModal component - will be used in the future
          <FlutterModal
            amount={parseFloat(amount) || 0}
            email={userData?.email || ""}
            phoneNumber={userData?.phoneNumber || "08000000000"}
          ...
          */}
          ...
          {/* Temporary button to show payment unavailable message */}
          <button
          ...
          >
            {`Proceed to Donate ₦${(parseFloat(amount) || 0).toLocaleString()}`}
          </button>
        </div>
      </div>
    </div>
