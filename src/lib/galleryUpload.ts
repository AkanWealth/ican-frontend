 ...
 */
export async function uploadGalleryImage(file: File, onProgress?: (progress: number) => void): Promise<string> {
  try {
    console.log('Starting gallery image upload...');
    
    // Generate a unique folder path for gallery images
    const galleryFolderPath = `gallery-images/${new Date().getFullYear()}/${new Date().getMonth() + 1}`;
    
    // Create a unique filename with timestamp and original name
    const uniqueFileName = `${Date.now()}-${file.name.replace(/\s+/g, '-')}`;
    const fullPath = `${galleryFolderPath}/${uniqueFileName}`;
    
    console.log('Gallery image upload details:', {
      fileName: fullPath,
      fileType: file.type,
    ...
    console.log('Presigned post data received for gallery upload');
    ...
    console.log('Gallery image uploaded successfully:', uploadResponse.status);
    ...
    console.log('Gallery image URL:', uploadedUrl);
 ...
 */
export function validateGalleryImage(file: File): { valid: boolean; message?: string } {
  ...
  if (file.size > MAX_SIZE) {
    return { 
      valid: false, 
      message: `File is too large. Maximum size is 5MB, your file is ${(file.size / 1024 / 1024).toFixed(2)}MB` 
    };
  }
